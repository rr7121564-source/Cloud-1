...
if ":::" in raw_dump:  
    ...
    WM_TEXT = parts[10] if len(parts) > 10 and parts[10] != "none" else None  
...
async def encode_phase(app, video_path, sub_path, logo_path, msg_id):  
    ...
    target_h = res_map.get(RESOLUTION, None)  
    if target_h: vid_h = target_h  
    ...
    for f in os.listdir("fonts"):  
        if f.lower().endswith(('.ttf', '.otf')):  
            font_path = os.path.abspath(os.path.join("fonts", f)).replace('\\', '/')  
            break  
    ...
    if TASK_TYPE == "compress":  
        ...
        vf_filters =[]  
        if target_h: vf_filters.append(f"scale=-2:{target_h}")  
        if WM_TYPE == 'text' and WM_TEXT:  
            text_esc = WM_TEXT.replace('\\', '\\\\').replace(':', '\\:').replace("'", r"\'")  
            if WM_POS == "top-left": x, y = "10", "10"  
            elif WM_POS == "top-center": x, y = "(w-text_w)/2", "10"  
            elif WM_POS == "top-right": x, y = "w-text_w-10", "10"  
            elif WM_POS == "center-left": x, y = "10", "(h-text_h)/2"  
            elif WM_POS == "center-right": x, y = "w-text_w-10", "(h-text_h)/2"  
            elif WM_POS == "bottom-left": x, y = "10", "h-text_h-10"  
            elif WM_POS == "bottom-center": x, y = "(w-text_w)/2", "h-text_h-10"  
            elif WM_POS == "bottom-right": x, y = "w-text_w-10", "h-text_h-10"  
            else: x, y = "w-text_w-10", "10"  
            drawtext_str = f"drawtext=text='{text_esc}':fontsize={target_wm_h}:x={x}:y={y}:fontcolor=white:shadowx=2:shadowy=2"  
            if font_path: drawtext_str = drawtext_str.replace("drawtext=", f"drawtext=fontfile='{font_path}':")  
            vf_filters.append(drawtext_str)  
              
        if vf_filters: cmd.extend(['-vf', ",".join(vf_filters)])  
        cmd.extend(['-map', '0:v', '-map', '0:a?', '-map', '0:s?', '-c:v', 'libx264', '-preset', PRESET, '-crf', CRF, '-c:a', 'copy', '-c:s', 'copy', '-progress', 'pipe:1', output])  
        ...
        if has_image_wm:  
            ...
            filter_complex.append(f"[1:v]scale=-1:{target_wm_h}[wm];{current_v}[wm]overlay={x}:{y}[outv]")  
            current_v = "[outv]"  
            ...
            drawtext_str = f"{current_v}drawtext=text='{text_esc}':fontsize={target_wm_h}:x={x}:y={y}:fontcolor=white:shadowx=2:shadowy=2[outv]"  
            if font_path: drawtext_str = drawtext_str.replace("drawtext=", f"drawtext=fontfile='{font_path}':")  
            filter_complex.append(drawtext_str)  
            current_v = "[outv]"  
    ...
    if returncode == 0 and os.path.exists(output):  
        ...
        try:  
            ...
            try:  
                ...
                try: await app.edit_message_text(CHAT_ID, msg_id, f"❌ Upload Error: {str(inner_e)}")  
                except: pass  
    else:  
